...
"""This script globs MosDepth output and compile per target and threshold coverage for all samples"""
...
def read_threshold(threshold_lst):
    ...
    for t in threshold_lst:
        ...
        sdf = sdf[
            ["%Over10X", "%Over20X", "%Over50X", "%Over100X", "%Over500X", "%Over1000X"]
        ]
        # print(sdf)
        sdf_lst.append(sdf)
...
def run():
    ...
    below_cov_threshold_df = pd.merge(
        no_neg_mdf, low_cov_df_target_names, on="target", how="inner"
    )

    mdf.to_excel(writer, sheet_name="All_Samples", index=True)

    below_cov_threshold_df.to_excel(
        writer, sheet_name=f"TargetsBelow_{cov_threshold}X", index=False
    )
